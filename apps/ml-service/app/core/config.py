...
class Settings:
    ...
    ocr_engine: str = os.getenv(
        "OCR_ENGINE",
        "tesseract",
    )
    detection_model_name: str = os.getenv(
        "DETECTION_MODEL_NAME",
        "yolov8n",
    )
    enable_segmentation: bool = (
        os.getenv("ENABLE_SEGMENTATION", "false").lower() == "true"
    )
    device: str = os.getenv("ML_DEVICE", "cpu")
    service_key: str = os.getenv("ML_SERVICE_KEY", "")
    trusted_storage_hosts: set[str] = {
        host.strip().lower()
        for host in os.getenv("TRUSTED_STORAGE_HOSTS", "").split(",")
        if host.strip()
    }
